...
<body>
   ...
   <br><br>   
    <form id = "myForm" action="LedgerProgramV6.php" method="post">
    ...
    <?php 
        ...
        echo ('<div id="Block"><h1>Blockchain Data: </h1></div><br />');          
        if (strlen(GetInputData())>=10){  
            $FileName = htmlspecialchars($_POST['FileName']);
            WriteToFile($FileName);
            $FileLinesArray=ReadFileToArray($FileName.".csv"); 
            printTransactions($FileLinesArray, $FileName); 
            PrintoutToUser("","","<br><a href='index.html'><font color='red'>Link Back Home</font></a>");          
        } 
        ...
        function WriteToFileData($FileNameToWriteTo, $Data){
            $FileToWriteTo = fopen($FileNameToWriteTo.".csv", 'a') or die("Unable to open file!");
        ...
        function printTransactions($ArrayName, $FileName){
                $counter = 1;   
                $CheckIfExecutedIfStatementForBlock = False; 
                $BlockNumber = 0;
                $HashesFromFileArray = ReadFileToArray($FileName."Hash.csv");
                array_pop($HashesFromFileArray);                 
                foreach ($ArrayName as $LineFromFile){   
                    If(strlen($LineFromFile) >= 5){
                        // echo("<div id='BlockDataAll'");
                        PrintoutToUser("yellow", "3", "<div id='BlockData'>Transaction " . $counter . " <font color='white'>" . $LineFromFile . "</font></div>");                                      
                        If ($counter % 2 == 0) {                         
                            ...
                            PrintoutToUser("green", "3", "End of Block " .$BlockNumber . ": ". $HashValueOfBothTransactions . "<br /><br />");  
                            $CheckIfExecutedIfStatementForBlock = TRUE;                                                            
                        }
                        // if($CheckIfExecutedIfStatementForBlock){
                        //     echo("</div><br />");
                        //     $CheckIfExecutedIfStatementForBlock = FALSE;
                        // }  else{
                        //     $CheckIfExecutedIfStatementForBlock = FALSE; 
